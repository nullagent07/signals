...
# Глобальный словарь для хранения состояния сигналов
signal_states = {}
...
async def analyze_pair(symbol):
    while True:
        try:
            ...
            previous_signal = signal_states.get(symbol)

            if current_signal != 0 and current_signal != previous_signal:
                signal_states[symbol] = current_signal  # Обновляем состояние сигнала
                print(f"Pair: {symbol}")
